...
season matching `CURRENT_SEASON` in `data-pipeline/config.py`, role filter "Tutti", and export to
Excel. Place the downloaded `.xlsx` file directly in this folder, whatever it's actually named -
the pipeline picks up the one `.xlsx` file it finds here rather than matching an exact filename.
